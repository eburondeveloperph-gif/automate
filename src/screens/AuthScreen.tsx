  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      if (err.message?.includes('auth/unauthorized-domain')) {
         setError('ACTION REQUIRED: Add this domain to Firebase Authorized Domains: ais-dev-uq4u5vgfptdf4prhjk3c4k-56203130379.asia-east1.run.app');
      } else {
              ...
              {isLogin ? (
                <div className="flex flex-col w-full gap-4">
                  ...
                  <div className="relative flex items-center justify-center my-2">
                    ...
                    <span className="relative bg-transparent px-3 text-[10px] uppercase tracking-widest text-white/20">or email</span>
                  </div>
                  ...
                  <form onSubmit={handleEmailAuth} className="flex flex-col gap-3 w-full">
                    <input 
                      ...
                      required
                      placeholder="Email"
                      className="w-full bg-white/5 border border-white/5 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-white/20 transition-all placeholder:text-white/20"
                    />
                    <input 
                      type="password" 
                      value={password}
                    ...
                    <button 
                      type="submit" 
                      disabled={loading}
                      className="w-full bg-white/10 text-white rounded-xl py-3 text-xs font-semibold hover:bg-white/20 transition-all disabled:opacity-50 mt-1 active:scale-95"
                    >
                      Sign In
                    </button>
                  </form>
                ...
                <div className="flex flex-col w-full gap-4">
                  <form onSubmit={handleEmailAuth} className="flex flex-col gap-3 w-full">
                    ...
                    <button 
                      type="submit" 
                      disabled={loading}
                      className="w-full bg-[#D4AF37] text-black rounded-full py-3 text-sm font-semibold hover:bg-[#D4AF37]/90 transition-all disabled:opacity-50 shadow-lg active:scale-95"
                    >
